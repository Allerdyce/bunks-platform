...
type FieldLookupEntry = {
  label: string;
  group: string;
  helper?: string;
};

const FIELD_LABEL_LOOKUP = FIELD_GROUPS.reduce<Partial<Record<keyof OpsDetailsInput, FieldLookupEntry>>>((acc, group) => {
  group.fields.forEach((field) => {
    acc[field.name] = { label: field.label, group: group.title, helper: field.helper };
  });
  return acc;
}, {});

const PRESET_FIELD_NOTES: Partial<Record<keyof OpsDetailsInput, string>> = {
  doorCodesDocUrl: "Guests tap this from the welcome + reminder emails when they need lock codes or arrivals help.",
  arrivalNotesUrl: "Paired with the 'Arrival overview' button (route tips, parking plans, contingency steps).",
  liveInstructionsUrl: "Used for the main 'Open live instructions' CTA that guests receive 24 hours before arrival.",
  recommendationsUrl: "Feeds the 'Browse recommendations' link inside marketing + reminder communications.",
  guestBookUrl: "Adds the 'Open the guest book' link for itineraries, FAQs, and saved tips.",
  checkInWindow: "Referenced wherever we mention arrival timing across confirmation + reminder touchpoints.",
  checkOutTime: "Referenced in departure reminders and the guest portal.",
};
            ...
            {selectedPreset && (
              <div className="space-y-5">
                ...
                <p className="text-xs text-slate-500">
                  <span className="font-semibold text-slate-700">Fill empty</span> touches only blank fields below.
                  <span className="ml-1 font-semibold text-slate-700">Overwrite</span> replaces every mapped field with the preset values shown here.
                </p>
                ...
                {selectedPreset.defaults && Object.entries(selectedPreset.defaults).length > 0 && (
                  <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 space-y-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">Preset mapping</p>
                      <p className="text-sm text-slate-500">
                        Review which global fields this preset populates before applying it.
                      </p>
                    </div>
                    <div className="space-y-3">
                      {Object.entries(selectedPreset.defaults).map(([fieldName, fieldValue]) => {
                        if (typeof fieldValue === "undefined" || fieldValue === null) {
                          return null;
                        }
                        const meta = FIELD_LABEL_LOOKUP[fieldName as keyof OpsDetailsInput];
                        const note = PRESET_FIELD_NOTES[fieldName as keyof OpsDetailsInput];
                        const value = String(fieldValue);
                        const isUrl = /^https?:\/\//i.test(value) || value.startsWith("/");
                        return (
                          <div key={fieldName} className="rounded-2xl border border-white/60 bg-white/80 p-3 shadow-sm">
                            <div className="flex flex-col gap-1">
                              <p className="text-sm font-semibold text-slate-900">
                                {meta?.label ?? fieldName}
                                {meta?.group && <span className="ml-2 text-xs text-slate-400">({meta.group})</span>}
                              </p>
                              {isUrl ? (
                                <a href={value} target="_blank" rel="noreferrer" className="text-sm text-violet-700 break-all hover:underline">
                                  {value}
                                </a>
                              ) : (
